...
def query_instances_by_regions(full_regions):
    ...
    for q in qs:
        ...
        for s in single_instances:
            s['myregion'] = q[0]
            s['mykey'] = ('keys/'+full_regions[s['myregion']]['key']
                          ) if s['myregion'] in full_regions else ''
            s['mycity'] = full_regions[s['myregion']
                                       ]['city'] if s['myregion'] in full_regions else ''
...
def run_new_instances(instances, n):
    ...
    for i in range(n):
        ...
        for k in full_regions:
            if '' == mink:
                mink = k
            if full_regions[k]['count'] < full_regions[mink]['count']:
                mink = k

        # run the instance
        print('\033[32;1m[+] Running instance in %s with region %s\033[0m' %
              (full_regions[mink]['city'], mink))

        s = 'aws ec2 run-instances --instance-type '+instance_type + \
            ' --launch-template  LaunchTemplateId=' + \
            full_regions[mink]['lt']+' --region '+mink
...
def terminate_all_instances(instances):
    ...
    for i in instances:
        ...
        print('\033[32;1m[+] Terminating instance in %s with region %s\033[0m' %
              (get_instance_city(i), get_instance_region(i)))
        s = 'aws ec2 terminate-instances --instance-ids ' + \
            i['InstanceId'] + ' --region ' + get_instance_region(i)
        qs.append(subprocess.Popen(s,  shell=True))
...
def bootstrap_all_instances(instances, over=True):
    ...
    os.system('cd '+NODE_DIR+';tar -czvf node.tar.gz *.c* *.h* _ecc_key Makefile;')
    ...
    for i in instances:
        ...
        if is_running_instance(i):
            ...
            print('='*80+'\n' + 'Bootstrapping: '+get_instance_city(i) +
                  ' region '+get_instance_region(i) + " ip  "+get_instance_ip(i))

            ip = get_instance_ip(i)
            key_pair_file = get_instance_key(i)

            # Copy the folder
            cmd_cf = 'scp -i '+key_pair_file+' -r ' + \
                NODE_DIR+'/node.tar.gz '+username+'@'+ip+':~/'

            # Install libraries && compile
            inlib = ''
            if not over:
                inlib += 'if [ ! -f '+NODE_DIR+'/Nodeaws ];then '
...
def check_bootstraped(instances):
    ...
    for i in instances:
        ...
        if is_running_instance(i):
            ...
            s = 'ssh -i "'+key_pair_file+'"  '+username+'@'+ip + \
                ' \'if [ -f ~/'+NODE_DIR + \
                '/Nodeaws ]; then echo yes; else echo no; fi\''
            qs.append((i, subprocess.Popen(
                s,  shell=True, stdin=PIPE, stdout=PIPE)))

    for q in qs:

        rd = q[1].communicate()[0].decode('ascii')
        if rd.find('yes') >= 0:
            print('\033[32;1m[+] Instance %s %s  bootstrapped\033[0m' %
                  (get_instance_city(q[0]), get_instance_ip(q[0])))
            countboot += 1
        else:
            print("\033[31;1m[-] Instance %s %s NOT bootstrapped\033[0m" %
                  (get_instance_city(q[0]), get_instance_ip(q[0])))
...
def kill_launched_nodes(instances):
    ...
    for i in instances:
        if is_running_instance(i):
            ...
            print('\t[ ] %s with %s ' %
                  (get_instance_city(i), get_instance_ip(i)))

            ip = get_instance_ip(i)
            k = 'cd '+NODE_DIR+'; fuser -k out*;'
...
def get_running_nodes(instances):
    ...
    for i in instances:
        if is_running_instance(i):
            ...
            full = 'ssh -i "' + \
                get_instance_key(i)+'"  '+username+'@'+ip+' \''+ps+'\''

            qs.append((i, subprocess.Popen(
                full,  shell=True, stdin=PIPE, stdout=PIPE)))
    ...
    for q in qs:
        ...
        print('Instance %15s %15s : %s' %
              (get_instance_city(q[0]), get_instance_ip(q[0]), rd))
...
def print_hashes(instances, only_one=-1):
    ...
    for i in instances:
        if is_running_instance(i):
            if only_one >= 0:
                c1 = " \"cd rust-randchain/_Hashes;find . -name '*' -type f -exec grep '^" + \
                    str(only_one)+":' {} \\;\""
            else:
                c1 = " \"cd rust-randchain/_Hashes;find . -name '*' -type f -exec tail -n 1 {} \\;\""
            c1 = "ssh -i " + \
                get_instance_key(i)+" "+username+"@"+get_instance_ip(i) + c1
            kl.append(subprocess.Popen(
                c1,  shell=True, stdin=PIPE, stdout=PIPE))
...
def get_dlat(instances):
    ...
    for i in instances:
        if is_running_instance(i):
            c1 = " cd rust-randchain; grep '^0' _Pings/*8* | awk -v sp=':' 'BEGIN{max0=0}{ if (\\$4>0+max0) max0=\\$4 fi;} END{printf max0 sp};';grep '^1' _Pings/* | awk 'BEGIN{max1=0}{if (\\$3>0+max1) max1=\\$3 fi;} END{printf max1};'"
            c1 = "ssh -i " + \
                get_instance_key(i)+" "+username+"@" + \
                get_instance_ip(i) + ' "' + c1 + '"'
            kl.append(subprocess.Popen(
                c1,  shell=True, stdin=PIPE, stdout=PIPE))
...
def check_pattern(instances, pattern):
    ...
    for i in instances:
        if is_running_instance(i):
            c1 = " 'cd rust-randchain; for filename in ./out*; do grep \"" + \
                pattern+"\" $filename|tail -1; done'"
            c1 = "ssh -i " + \
                get_instance_key(i)+" "+username+"@"+get_instance_ip(i) + c1
            kl.append(subprocess.Popen(c1, shell=True, stdin=None,
...
def relaunch_nodes(instances, impose_limit=True, rate_Mbps="40"):
    ...
    print('='*80+'\n' + 'Relaunching ::: throughput_limit %d ::: rate %s Mbps' %
          (impose_limit, rate_Mbps))

    # create the necessary files, i.e. _peers_ip, _configuration, _network
    create_network_config_files(instances, CONFIG_FILE)
    ...
    for i in instances:
        if is_running_instance(i):
            ...
            c1 = 'scp -i "'+ikey+'" '+CONFIG_FILE+' ' + NETWORK_FILE + \
                ' ' + PEERS_FILE + ' '+username+'@'+ip+':~/'+NODE_DIR+'/'
            ...
            if impose_limit:
                ...
                c1 += "     sudo tc qdisc add dev $rdev handle $i: parent 1:$i tbf rate " + \
                    rate_Mbps+"Mbit burst 500k latency 10000s;"
                c1 += "     sudo tc filter add dev $rdev pref $i protocol ip u32 match ip dport $(("+str(
                    start_port)+"+$i-2)) 0xffff flowid 1:$i;"
                c1 += "done;"

            c1 += "cd rust-randchain; fuser -k out*; rm out*; rm -rf _Blockchains/*;rm -rf _Sessions/*;rm -rf _Hashes/*;rm -rf _Pings/*;rm -rf _Blocks/*;for((i="+str(start_port)+";i<"+str(start_port+nodes)+";i++))\
...
def special_relaunch_nodes(instances, impose_limit=True, rate_Mbps="40"):
    ...
    print('='*80+'\n' + 'SPECIAL Relaunching ::: throughput_limit %d ::: rate %s Mbps' %
          (impose_limit, rate_Mbps))
    ...
    for i in instances:
        if is_running_instance(i):
            ...
            if impose_limit:
                ...
                c1 += "     sudo tc class add dev $rdev parent 1:1 classid 1:$i htb rate 1000Mbps;"
                c1 += "     sudo tc qdisc add dev $rdev handle $i: parent 1:$i tbf rate " + \
                    str(rate_Mbps)+"Mbit burst 500k latency 10000s;"
                #c1 += "     sudo tc filter add dev $rdev pref $i protocol ip u32 match ip sport $(("+str(start_port)+"+$i-2)) 0xffff flowid 1:$i;"
                c1 += "     sudo tc filter add dev $rdev pref $i protocol ip u32 match ip dport $(("+str(
                    start_port)+"+$i-2)) 0xffff flowid 1:$i;"
                c1 += "done;"

            c1 += "cd rust-randchain; fuser -k out*; rm out*; rm -rf _Blockchains/*;rm -rf _Sessions/*;rm -rf _Hashes/*;rm -rf _Pings/*;rm -rf _Blocks/*;"
    ...
    for i in instances:
        if is_running_instance(i):
            if count == special_node:
                ...
                c1 += ";ssh -i "+ikey+" "+username+"@"+ip+" "
                c1 += "'"
                c1 += "cd rust-randchain;"
                c1 += "nohup ./Nodeaws " + \
                    str(special_port)+" _network "+ip+" " + \
                    ip_private+" >> out"+str(special_port)+".txt &"
                c1 += "'"
                kl.append(subprocess.Popen(c1, shell=True, stdin=None,
                                           stdout=None, stderr=None, close_fds=True))
...
def dump_data(instances):
    ...
    for i in instances:
        if is_running_instance(i):
            ...
            c1 = 'ssh -i "' + \
                get_instance_key(i)+'"  '+username+'@'+get_instance_ip(i)
            c1 += ' \'cd ~/'+NODE_DIR+'; tar czf ' + \
                get_instance_ip(i)+'.tar.gz out* '
            c1 += '_Hashes '
            c1 += '_Pings '
            c1 += '_Blocks '
            c1 += '\''
            c1 += '; scp -i "'+get_instance_key(i)+'" -r '+username+'@'+get_instance_ip(
                i)+':~/'+NODE_DIR+'/'+get_instance_ip(i)+'.tar.gz ' + full_folder
            kl.append(subprocess.Popen(c1,  shell=True, stdin=None,
                                       stdout=None, stderr=None, close_fds=True))
...
def dump_blocks(instances):
    ...
    for i in instances:
        if is_running_instance(i):
            ...
            c1 = ' scp -i "'+get_instance_key(i)+'" -r '+username+'@'+get_instance_ip(
                i)+':~/'+NODE_DIR+'/_Blocks '+full_folder+"/"
...
def list_instances(instances):
    ...
    for i in instances:
        ...
        if 'State' in i and 'Name' in i['State']:
            st = i['State']['Name']
        print('[+] Instance %15s with %14s is %s ' %
              (get_instance_city(i), get_instance_ip(i), st))
